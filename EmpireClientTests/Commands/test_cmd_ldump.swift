...
final class test_cmd_ldump: XCTestCase {
    ...
    func test_parse_ldump() throws {
        ...
        XCTAssertEqual(g.landUnits[0]?.attack, 1.65)
        XCTAssertEqual(g.landUnits[0]?.defense, 0.69)
        XCTAssertEqual(g.landUnits[0]?.vulnerability, 71)
        XCTAssertEqual(g.landUnits[0]?.speed, 38)
        XCTAssertEqual(g.landUnits[0]?.visibility, 18)
        XCTAssertEqual(g.landUnits[0]?.spy, 4)
        XCTAssertEqual(g.landUnits[0]?.radius, 3)
        XCTAssertEqual(g.landUnits[0]?.frg, 19)
        XCTAssertEqual(g.landUnits[0]?.accuracy, 20)
        XCTAssertEqual(g.landUnits[0]?.damage, 21)
        XCTAssertEqual(g.landUnits[0]?.ammoUse, 22)
